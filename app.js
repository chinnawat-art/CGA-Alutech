...
function normalizePaymentTime(value) {
    if (value === null || value === undefined) return null;
    // If it's a DOM input element (e.g., accidentally passed), extract .value
    try {
    ...
    if (typeof value === 'string') {
        const s = value.trim();
        if (s === '') return null;
        // If looks like ISO already, try Date
        const d = new Date(s);
        if (!isNaN(d.getTime())) return d.toISOString();
        // Try to parse common formats by replacing space with T
    ...
    function handleFileSelect(file) {
        ...
        reader.onload = async function(e) {
            try {
                ...
                // Try to robustly detect header row. Some Shopee/Lazada exports have extra rows above header.
                const rows = XLSX.utils.sheet_to_json(worksheet, {header: 1, defval: ""});
                let rawData = [];

                // Keywords that likely appear in the header row
                const headerKeywords = ['หมายเลขคำสั่งซื้อ','order','sku','ชื่อสินค้า','วันที่ทำการสั่งซื้อ','เวลาการชำระ','paid','payment','หมายเลขติดตามพัสดุ'];

                // Search top 10 rows for the best header candidate
                ...
                if (bestIdx >= 0 && bestScore > 0) {
                    ...
                    console.log('[DEBUG] auto-detected header row index:', bestIdx, 'header:', headerRow);
                    // Build objects for subsequent rows
                    ...
                    // Fallback: let sheet_to_json pick the headers (original behavior)
        ...
        for (let k of keys) {
            ...
            if (headerMapCache.has(normalizedK)) {
                ...
                if (val !== undefined && val !== null && val !== '') {
                    // Preserve Date objects so normalizePaymentTime can handle them
            ...
            // Fallback: try substring match (handles headers with extra notes or whitespace)
            const substringKey = rowKeys.find(rk => rk.toLowerCase().trim().includes(normalizedK) || normalizedK.includes(rk.toLowerCase().trim()));
            if (substringKey) {
    ...
    async function processExcelData(rawData) {
        ...
        try {
            ...
            if (paymentCandidateIndex !== -1) {
                ...
                paymentKeyVariants.forEach(k => headerMapCache.set(String(k).toLowerCase().trim(), candidateKey));
        ...
        for (let i = 0; i < cleanData.length; i++) {
            ...
            if (platform === 'Lazada') {
                ...
                // Payment time (Lazada)
                var paymentRaw = getValue(row, ['paidTime', 'paymentTime', 'payment_date', 'paid_at', 'เวลาการชำระเงิน', 'เวลาชำระเงิน', 'เวลาการชำระสินค้า'], rowKeys);
                ...
                // Payment time (Shopee)
                var paymentRaw = getValue(row, ['paid_time', 'paidTime', 'payment_time', 'paymentDate', 'วันที่ชำระเงิน', 'เวลาการชำระเงิน', 'เวลาชำระสินค้า', 'เวลาการชำระเงิน (ตามร้านค้า)'], rowKeys);
                let paymentTime = normalizePaymentTime(paymentRaw || null);
        ...
        bulkUploadBtn.addEventListener('click', async () => {
            ...
            try {
                ...
                const orderNumbers = parsedDataToUpload
                    .map(o => normalizeString(o.order_number))
                    .filter(Boolean);
                ...
                parsedDataToUpload.forEach(item => {
                    const orderNumberKey = normalizeMatchPart(item.order_number);
                    ...
                    const existing = (t ? existingByTracking.get(normalizeMatchPart(t)) : null)
                ...
                if (itemsToInsert.length === 0 && itemsToUpdate.length === 0) {
                    ...
                    alert(`⚠️ ไม่มีรายการที่สามารถบันทึกได้\n\nรายการออเดอร์เดิมที่สถานะยังไม่เป็น \"ผลิตสำเร็จแล้ว\": ${skipCount} รายการ${sampleOrders ? `\nตัวอย่าง: ${sampleOrders}` : ''}`);
                    resetBulkUI();
                    return;
                }

                log(`เตรียมการ: อัปเดตข้อมูลเก่า ${itemsToUpdate.length} รายการ, บันทึกใหม่ ${itemsToInsert.length} รายการ, บล็อกเพราะยังผลิตไม่สำเร็จ ${skipCount} รายการ...`);
