...
async function loadProductImage(sku) {
    ...
    // Check cache first
    if (productImageCache[normalizedSku]) {
    ...
    try {
        ...
        if (error) {
            console.warn(`Product image lookup error for SKU ${normalizedSku}:`, error);
            return null;
        }
...
async function loadData() {
    ...
    const selectColumns = 'id,order_date,platform,order_number,production_number,tracking_number,product_code,product_name,product_size,slots,quantity,buyer_name,tracking_status,note,pattern,production_started_at,production_completed_at,payment_time,aluminum_color,glass_color,screen_type,stock_deducted';
...
function openComponentsModal(order, comps, backupOption = null) {
    return new Promise((resolve) => {
        ...
        confirmBtn.onclick = () => onDone('components');
...
function getProductionSequenceForOrder(order) {
    if (!order?.order_date || !Array.isArray(allOrders)) return 1;
    const targetDate = new Date(order.order_date);
    ...
    const sameDayOrders = allOrders
        ...
        .sort((a, b) => {
            const payA = a.payment_time ? new Date(a.payment_time).getTime() : Number.MAX_SAFE_INTEGER;
            const payB = b.payment_time ? new Date(b.payment_time).getTime() : Number.MAX_SAFE_INTEGER;
            if (payA !== payB) return payA - payB;

            const orderNumberCompare = String(a.order_number || '').localeCompare(
                String(b.order_number || ''),
                'th',
                { numeric: true }
            );
            if (orderNumberCompare !== 0) return orderNumberCompare;
...
function formatProductionNumberLabel(order) {
    ...
    if (order?.order_date) {
        ...
        if (!isNaN(date.getTime())) {
            ...
            const seq = getProductionSequenceForOrder(order);
            return `${seq}/${day}`;
        }
    }
...
function renderOrderCard(o) {
    ...
    // Load product image asynchronously in the background
    const imagePromise = loadProductImage(o.product_code);
    ...
    // Load product image and insert into container with improved styling
    const imagePromiseForUpdate = loadProductImage(o.product_code).then(imageUrl => {
        setTimeout(() => {
            if (imageUrl) {
            ...
            <div style="text-align:right;">
                ...
                <div class="card-date" style="margin-top:4px;">${(() => {
                    try {
                        ...
                        // If payment_time exists, append it on next line (formatted time)
                        if (o.payment_time) {
                            try {
                                const pd = new Date(o.payment_time);
                                if (!isNaN(pd.getTime())) {
...
function renderCards() {
    ...
    // Apply search query from searchInput (if present)
    try {
...
async function deleteProductionOrder(id, btnEl) {
    ...
    try {
        if (order.tracking_status === STATUS_DONE) {
            const backupRemoved = await removeBackupStockForRevertedOrder(order);
            if (!backupRemoved) throw new Error('ย้อนสต็อกสินค้าสำเร็จรูปไม่สำเร็จ');
        }
        ...
        log(
            `🗑️ ลบออเดอร์ ${order.order_number || id} พร้อมรายงานวัสดุเสียและรายการจ่ายวัสดุแล้ว` +
            ` (วัสดุเสีย ${cleanupResult?.deleted_damage_rows || 0} รายการ, วัสดุ ${cleanupResult?.deleted_material_rows || 0} รายการ)`,
            'success'
        );
...
async function updateStatus(id, newStatus, btnEl) {
    ...
    try {
        ...
        const usedBackupStock = isRevertingFromDone
            ? await hasBackupAllocationForOrder(order.id)
            : false;
        if (isRevertingFromDone) {
            if (usedBackupStock) {
                await rpcReturnBackupStockForOrder(order);
            } else {
        ...
        if (shouldReturn) {
            const returnedStatus = usedBackupStock ? true : await performReturn(order);
            if (returnedStatus) {
...
function setupRealtime() {
    ...
    db.channel('prod:stock_orders')
        .on('postgres_changes', { event: '*', schema: 'public', table: TABLE }, payload => {
            ...
            if (eventType === 'INSERT') {
                if (canViewProductionStatus(nw.tracking_status) && !nw.tracking_number) {
                    allOrders.push(nw);
