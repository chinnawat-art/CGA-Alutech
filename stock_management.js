...
function getFiltered() {
    ...
    return allStocks.filter(s => {
        ...
        const isLow = (Number(s.quantity) || 0) <= getReorderPoint(s);
...
function buildRow(stock) {
    const isLow = (Number(stock.quantity) || 0) <= getReorderPoint(stock);
    ...
    try {
        if (currentEditingId) {
            ...
            const updateData = {
                ...
                reorder_point: parseInt(document.getElementById('reorderPoint').value, 10),
                unit:         unit,
                quantity:     newQty,
